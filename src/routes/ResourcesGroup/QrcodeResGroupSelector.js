...
@connect(({ resourcesgroup, loading }) => ({
  resourcesgroup,
  loading: loading.effects['resourcesgroup/listAllResList'],
}))
export default class QrcodeResGroupSelector extends PureComponent {
  state = {
  ...
  componentWillMount() {
    this.props.dispatch({
      type: 'resourcesgroup/listAllResList',
      payload: {
  ...
  downloadZip = () => {
    ...
    a.href = `${domain_api}/api/admin/v1/qrcode/resources?groupIds=${ids}`;
    a.target = '_blank';
    a.click();
  }
  columns = [{
  ...
  render() {
    const previewQrCodeModal = (
      <Modal
        ...
        footer={null}
      >
        <img style={{ height: '100%', width: '100%' }} src={`${domain_api}/api/admin/v1/qrcode/resources/${this.state.currentSelectedId}`}/>
      </Modal>
    );
    const rowSelection = {
      ...
      <div>
        <Card border="false">
          ...
          <Table
            ...
            dataSource={this.props.resourcesgroup.resList}
          />
        </Card>
