...
@connect(({record, loading}) => ({
  record,
  loading: loading.effects['record/listResourceGroups'],
}))
@Form.create()
export default class ResourceGroups extends PureComponent {
  state = {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.props.dispatch({
          type: 'record/listResourceGroups',
          payload: {
            startDay: start,
            endDay: end,
            groupIds,
  ...
  render() {
    const { data, fields } = this.transferData(this.props.record.resourceGroups);
      ...
      <div>
        <Card border="false">
          ...
          >
            ...
            <FormItem style={{ marginBottom: 0 }}>
              {getFieldDecorator('selectedGroupIds')(
                <Select
                  mode="multiple"
